...
# #-------------------------------#
# #triginometry in numpy

# print(np.pi)#3.141592653589793

# print(np.sin(np.pi/2)) #angle should be in radians

# print(np.sin(np.array([np.pi/2, np.pi/3, np.pi/4, np.pi/5]))) #values of angle fron an array

# # there are cos() and tan() functions as well
...
# #als oyou can convert from array like we did in sin
...
# There are hyperbolic functions such as sinh cosh and tanh also arcsinh arcconh and arctanh
...
#as you can see this is not a set as there are duplicates so to remove those we can use np.unique()

print(f"Originial array:{arr}")
arr=np.unique(arr)
print(f"Set array:{arr}")

#set operations on 1d array

set1=np.array([1,2,3,4,5])
set2=np.array([3,4,5,6,7])

#union
print(np.union1d(set1,set2))#[1 2 3 4 5 6 7]
print(np.intersect1d(set1,set2))#[3 4 5]
print(np.setdiff1d(set1,set2)) #[1 2]

# To find only the values that are NOT present in BOTH sets
print(np.setxor1d(set1,set2)) #[1 2 6 7]
